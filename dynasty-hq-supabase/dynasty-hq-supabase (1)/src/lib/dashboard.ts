...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  // ---- 2. Assets + Graphics (static reference data, not dynasty-scoped) ----
  ...
  const { data: graphicsRows, error: graphicsErr } = await sb.from('graphics').select('*');
  if (graphicsErr) throw new Error(`graphics: ${graphicsErr.message}`);
  ...
  const result: DashboardData = {
    ...
    team: myAsset,
    opponent: oppAsset,
    assets: (assetRows || []).map(toTeamAsset),
    graphics: (graphicsRows || []).map((r: any) => ({ conference: r.conference, abbreviation: r.conference_abbreviation, logoUrl: r.logo_url })),
